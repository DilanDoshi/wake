...
// The manager wears yellow by default - on its room turns and its roster row -
// so the fleet's coordinator stands apart from the shared Accent the ordinary
// agents default to, without anyone running /color. The status bar is not one
// of these surfaces: it recedes as chrome regardless of colour (statusbar.go).
