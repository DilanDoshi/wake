...
// The other two envelopes Claude wraps a slash command's transcript entry in
// are plumbing, not conversation, and unlike the stdout one there is nothing
// inside to unwrap: the <command-name>…</command-args> invocation echo and the
// <local-command-caveat> banner (meant for the model, isMeta on disk). Both are
// dropped whole, so a reopened transcript never draws the raw tags as a turn.
// The command's own output is a separate frame and is still shown.
func TestUserTextDropsLocalCommandPlumbingEnvelopes(t *testing.T) {
	cases := map[string]struct {
		content string // the JSON value of message.content, quotes included
		want    int    // events after decode
	}{
		"invocation dropped": {`"<command-name>/clear</command-name>\n            <command-message>clear</command-message>\n            <command-args></command-args>"`, 0},
		"caveat dropped":     {`"<local-command-caveat>Caveat: the messages below were generated by the user while running local commands.</local-command-caveat>"`, 0},
		"ordinary text kept": {`"just text"`, 1},
		// Only a whole envelope is dropped; a stray opening tag is content.
		"half an envelope kept": {`"<command-name>/clear"`, 1},
	}
	for name, c := range cases {
		t.Run(name, func(t *testing.T) {
			line := `{"type":"user","session_id":"s1","message":{"role":"user","content":` + c.content + `}}`
			if got := len(decodeOne(t, line)); got != c.want {
				t.Fatalf("decoded %d events, want %d", got, c.want)
			}
		})
	}
}

// ...and never from the assistant's side, the only side these have never
// appeared on: dropping there would silently delete agent speech, so the drop
// is gated to a user frame.
func TestAssistantTextKeepsTheLocalCommandInvocationEnvelope(t *testing.T) {
	line := `{"type":"assistant","session_id":"s1","message":{"role":"assistant","content":"<command-name>/clear</command-name>\n            <command-args></command-args>"}}`
	ev := onlyEvent(t, line, 0)
	if ev.Kind != KindAssistantText {
		t.Fatalf("Kind = %q, want %q", ev.Kind, KindAssistantText)
	}
...
// ...and never from array content: the same tags in a text block are agent prose
// or a pasted quote, exactly as crossSession leaves array content alone. The
// discriminator is the bare string the CLI writes a real envelope as.
func TestAnArrayContentUserFrameKeepsTheLocalCommandTags(t *testing.T) {
	line := `{"type":"user","session_id":"s1","message":{"role":"user","content":[{"type":"text","text":"see <command-name>/clear</command-name> and <command-args></command-args>"}]}}`
	ev := onlyEvent(t, line, 0)
	if ev.Kind != KindUserText {
		t.Fatalf("Kind = %q, want %q: array content is never plumbing", ev.Kind, KindUserText)
	}
}

// The bug this fixes is a reopen from disk, so the drop must hold on the
// transcript path too - DecodeTranscriptLine in front of DecodeLine, the one
// decoder live and history share.
func TestDecodeTranscriptDropsLocalCommandPlumbing(t *testing.T) {
	for _, content := range []string{
		`<command-name>/model</command-name>\n            <command-args></command-args>`,
		`<local-command-caveat>Caveat: generated while running local commands.</local-command-caveat>`,
	} {
		line := []byte(`{"type":"user","userType":"external","isSidechain":false,"sessionId":"s1","message":{"role":"user","content":"` + content + `"}}`)
		evs, err := DecodeTranscriptLine(line)
		if err != nil {
			t.Fatalf("DecodeTranscriptLine: %v", err)
		}
		if len(evs) != 0 {
			t.Errorf("got %d events, want 0 (dropped): %+v", len(evs), evs)
		}
	}
}
