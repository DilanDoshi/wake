...
var notInTheStatusReport = map[string]string{
	...
	"Team":           "the team an operator grouped this agent under, Color's neighbour and withheld with it: FrameTeam is refused the manager for the same whose-decision reason, so a team is a fact this surface cannot act on. It is the reversible default rather than a settled one - unlike a colour a team routes work (@team fans out), so there is a real case for showing the manager one read-only, and that case is the owner's to decide (deferred.md). Until then it is withheld, because exposing a value is additive and un-exposing one is a capability removal",
...
const notInTheStatusReportCount = 13
