...
| The conversation's status bar | `internal/ui/statusbar.go` — path, branch, model, context left, **effort**, and the permission mode; cached on `DM.bar`, drawn per change, and drawn inside the composer below the box (above the armed cue on the rare frame one is up). **The permission mode is the bar's alone now** — the always-on legend that used to carry it is gone, so `modeFormat` lives here. Effort is the level `confirmedEffort` reads back, or the asked-for one until then. The mode is **drawn whole or dropped**, never right-cut into `permissions: …` |
