...
| A dynamic workflow, decoded | `internal/core/workflow.go` (Wake's own vocabulary, not an airlock file — `WorkflowUpdate`, `WorkflowSnapshot` (`Done`, `PhaseAgents`), `WorkflowPhase`, `WorkflowAgentState` (`Running`/`Done`/`Failed`/`Unknown` — `Running` covers both recorded in-progress words, `start` and `progress`; `Failed` is the SDK's documented word for an individual agent failure this corpus has not recorded), `WorkflowAgent`, `WorkflowRun`, the disk record's own shape) · `encode.go` — the wire structs, `workflowSnapshotOf` (a `workflow_progress` array to a snapshot; every frame is a full replacement, never a delta), `workflowOf` (the recogniser, here for room — `goalOp`/`toolLoopOp`'s precedent), `DecodeWorkflowRun` (one `workflows/wf_*.json` record), `EncodeStopTask` (`stop_task`, addressed by the run's own task id — pause and restart have no wire form at all, findings §6) · `write.go` — `Session.StopTask`, recording nothing, `SetMode`'s own shape · `protocol.go` — `taskUpdate`'s one added field · `vocabulary.go` — `TaskWorkflow`/`TaskFailed`, both now recorded words · `DecodeSidechainLine` — `DecodeTranscriptLine` minus the sidechain drop, one shared body, since every line of a workflow agent's own transcript is `isSidechain:true` · `internal/rpc/workflow.go` — the seven frame kinds and `WorkflowFrame`, `ValidWorkflowName`/`ValidWorkflowAgentID` (the two fences), `ScopeProject`/`ScopeUser` (spelled `"personal"` on the wire, not Claude's own `"user"` — the message role's word, policed by `airlock_test.go`) |
...
- **Small files: 200–400 typical, 800 hard max.** The two largest non-test files are
  `internal/core/protocol.go` at 800 and `internal/core/vocabulary.go` at 800 — that sentence is
