...
| Handing a newly attached client the fleet's outstanding asks | `internal/daemon/askreplay.go` — `replayPendingAsks`, called before `addClient` in `server.go`'s `run` so it can never race a broadcast of the same ask · `internal/daemon/agent.go` — `ask.event` (the retained `core.Event`), `pendingAskFrames`. Without this a client that attached after an ask went past learned only `rpc.SessionStatus.RequestIDs`, from which `internal/ui.Cards.Reconcile` could build only a bare permission stand-in — silently wrong for a question, whose `Allow` is then a bare `FrameAllow` |
