...
| Inline completion, and the directory read that is not on the draw goroutine | `internal/ui/completion.go` — `completing`, `completionUp` (pane *and* draft), `completionKey` (⇥ · ⌃N/⌃P, read above `App.key`'s switch so they take no legend entry; `↑↓` walk it too, in keys.go's `KeyUp`/`KeyDown` after the cursor-move guard), `addressees` (the `@` menu's names: agents, then the fleet's teams, then `@all`; `completion.teams`/`label` tag a team `@backend (team)` in the draw without changing what an accept inserts) · `completionpath.go` — `scanning`, `pathsScanned`, one directory, `pathScanMax` entries, one read at a time · `slash.go`'s `commandStem`/`wakeVerbs`, because only that file knows what a leading slash means · `completionteam_test.go` |
