...
a key to that legend until the key works.** The whole legend needs **332 columns**; below that the
mode is cut first, deliberately, because a key is something to do and the mode is something to know.
Drawing all of it needs a terminal at least **372** columns wide with both sidebars open, so every
