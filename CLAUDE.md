...
| A subagent | Its work is a conversation of its own, not a paragraph in yours: the pane lists what it dispatched — type, what it is doing now, elapsed, tokens — and `↵` on a row opens that subagent's transcript in the pane. `↵` on the conversation's own row is the way back. **The right sidebar lists the ones still running**, indented under the agent that dispatched them, named by `subagent_type` with what each has spent; `↑↓` walk onto one and `⌃D` opens it. Running only, and the pane keeps the finished ones — two surfaces, two questions |
...
a key to that legend until the key works.** The whole legend needs **352 columns**; below that the
mode is cut first, deliberately, because a key is something to do and the mode is something to know.
Drawing all of it needs a terminal at least **392** columns wide with both sidebars open, so every
...
| The board: the fleet as one row per agent | `internal/ui/board.go` — `/board`, an overview and never panes (the owner's 2026-08-12 ruling); drawn instead of the grid, closed by any key it does not claim |
...
| What a conversation has dispatched | `internal/ui/tasks.go` — the fold, pure, and `named` (the ending frame does not say what ended) · `taskrows.go` — the list under the pane · `taskkeys.go` — `⌃N`/`⌃P` and what `↵` opens · `taskline.go` — the line an ending leaves in the transcript |
| Who owns that fold, and why it is not on `Agent` | `internal/ui/fleettasks.go` — `Fleet.tasks` keyed on session id, `RunningTasks` (the sidebar's filter), `named` (ingest-time enrichment). It is a second map because `Agent` must stay comparable for `Observe`'s `now == was`; `DM.tasks` is a **projection set by `App.dmFor`**, never folded into |
...
| The live checklist: decode, fold, snapshot | `internal/core/vocabulary.go` — `toolChecklistOp` (one `TaskCreate`/`TaskUpdate` op, `TodoWrite` retired) · `internal/ui/checklist.go` — the `checklist` type (id-keyed on claude's monotonic counter, not position), `Fleet.foldChecklist` (the live working line) and `DM.foldChecklist` (the transcript, folded in `Append` and re-derived in `Before` so a list survives a restore off disk). The Fleet fold attaches the snapshot to `ToolCall.Todos` for the working line; the DM stores only the compact op and `withSnapshot` rebuilds the list at render time (`renderAll`), so a run of ops does not pin a copy of the whole list each (BUG-26). `render.TodoList` and `foldExempt` are unchanged |
