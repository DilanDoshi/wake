...
| One agent's permission asks: the type and its bookkeeping | `internal/daemon/asks.go` — `ask`, `addPending`, `noteAnswered`, `awaitsChoice`, `blockedOnAsk`, `pendingIDsLocked`, `withoutAsk` (split from `agent.go` at the 800-line hard max) |
