...
| The native `/loop`, decoded and rendered | `internal/core/loop.go` (Wake's vocabulary — `LoopKind`, `LoopOp`) · `encode.go` — `toolLoopOp` (the recogniser: a recurring `CronCreate` is fixed, `ScheduleWakeup` is self-paced, `CronDelete` ends it; here for room, beside `goalOp`) · `internal/daemon/loop.go` — `foldLoop`, `loopStatus` · `internal/ui/loop.go` — `LoopState`, `withLoop`, `loopLine`/`loopCadence`, the `↻` glyph · rendered by `roster.go` (marker + activity line), `statusbar.go` (segment, on the DM and room bars), `board.go` · `testdata/stream/loop-{fixed,selfpaced}.jsonl`. Headless the `/loop` slash is not a command — the model reproduces it with the scheduler tools — so Wake reads those tool calls; an ended loop is silent beyond a `CronDelete` (the loop half of the goal's §6) |
