...
func (w *WordWrap) Write(b []byte) (int, error) {
	...
	for _, c := range s {
		if c == '\x1B' {
			...
			w.addSpace()
			w.addWord()
			// A breakpoint occupies a cell like any other rune, and it is
			// written straight to the buffer rather than held in w.word - so
			// it needs the limit check the ordinary-character branch does
			// below, and it needs counting afterwards. Upstream does neither,
			// so a line is measured short and then overruns. See WAKE-PATCH.md.
			if w.lineLen > 0 && w.lineLen+1 > w.Limit {
				w.addNewLine()
			}
			w.lineLen++
